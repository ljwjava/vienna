...
class Main extends React.Component {
    ...
    componentDidMount() {
        Proposal.fullView(this.state.proposalId, r => {
            this.setState({ proposal: r }, this.onInsurantSwitch.bind(this, 0))
        })
    }
    onInsurantSwitch(index) {
        Proposal.format(this.state.proposal.detail[index], "liab_graph,coverage,benefit_chart", r => {
          this.setState({ index:index, plan:this.state.proposal.plans[index], coverage:r.coverage, chart:r.benefit_chart, liability:r.liab_graph })
        })
    }
